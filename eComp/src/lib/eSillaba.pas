...
var
  ...
  begin
    Result:= UpCase(Parola[ps]) in ['A','E','I','O','U','Y','à','è','é','ì','ò','ù'];
  end;
  function IsSemiConsonante(ps: integer): boolean;
  begin
    Result:= UpCase(Parola[ps]) in ['I','U','Y'];
  end;
  ...
  begin
    Result:= (Upcase(Parola[ps])='G') and (Upcase(Parola[ps+1]) in ['L','N']);
  end;
  ...
  begin
    Result:= (Upcase(Parola[ps+1]) in ['H','L','R']) and
      (not (Upcase(Parola[ps]) in ['N','M','H','L','R']));
  end;
